...
#AUC
def plot_figure1(task='response'):
    ...
    saving_dir = join(PLOTS_PATH, 'figure1_model_arch_{}'.format(task))
...
def plot_figure3_compare_domain_adaptation(task = 'response'):
    ...
    plt.legend(loc="lower right", fontsize=8, framealpha=0.0)
    plt.title(title, fontsize=10)
    saving_dir = join(PLOTS_PATH, 'figure3_domain_adaptation_{}'.format(task))
    if not exists(saving_dir):
        os.mkdir(saving_dir)
...
def plot_figure2_compare_model_sizes(task='response',tuned=False ):
    ...
    plt.legend(loc="lower right", fontsize=8, framealpha=0.0)
    plt.title(title, fontsize=10)
    saving_dir = join(PLOTS_PATH, 'figure2_model_size_{}_{}'.format(task, tuned))
    if not exists(saving_dir):
        os.mkdir(saving_dir)
    filename= join(saving_dir, '_auc')
    plt.savefig(filename, dpi=400)
...
plot_figure1('response')
plot_figure1('progression')
plot_figure2_compare_model_sizes(task='response',tuned=False )
plot_figure2_compare_model_sizes(task='progression',tuned=False )
plot_figure3_compare_domain_adaptation('response')
plot_figure3_compare_domain_adaptation('progression')
