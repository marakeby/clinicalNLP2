...
class BerttClassificationHead(nn.Module):
...
class CNN_Over_BERT(nn.Module):
    ...
    def __init__(self, bert, nhid, nlabels):
    ...
    def forward(self,
        ...
        return x, bert_outputs



class CNNOverBertSequenceClassification(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        ...
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier = CNN_Over_BERT(self.bert, config.hidden_size, config.num_labels)
        # self.classifier = nn.Linear(config.hidden_size, config.num_labels)
