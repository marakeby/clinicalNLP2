...
training_args = TrainingArguments(
        output_dir=join(LOG_PATH,f'DFCI_BERT/unfrozen/{fname}'),  # output directory
        num_train_epochs=10,  # total number of training epochs
        per_device_train_batch_size=32,  # batch size per device during training
        per_device_eval_batch_size=32,  # batch size for evaluation

        logging_dir=join(LOG_PATH,f'DFCI_BERT/unfrozen/{fname}'),  # directory for storing logs
        logging_steps=300,
       save_steps= 1000000,
...
#     'type': 'bert',
#     'id': meta_data['Model'],
#     'params': dict(bert_model_name = bert_size_tuned[meta_data['Size']],
...
feature_selection =[]
models = [bert_rnn, bert_cnn, bert_linear]
